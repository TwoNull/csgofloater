...
async function scrapeMarketPage(start: number, hashName: string) {
    const url = `https://steamcommunity.com/market/listings/730/${encodeURI(hashName)}/render?query=&start=${start}&count=100&currency=${1}&country=US&language=english&filter=`
