...
import { guardCodePrompt, buyPrompt, logo, colorize } from '../interface/interface';
...
export async function automate(results: any, data: any) {
    ...
    const payload = `sessionid=${sessionid}&currency=1&subtotal=1&fee=1&total=1&quantity=1&first_name=${CONFIG.billing.first_name}&last_name=${CONFIG.billing.last_name}&billing_address=${steamifyString(CONFIG.billing.billing_address_1)}&billing_address_two=${steamifyString(CONFIG.billing.billing_address_1)}&billing_country=${CONFIG.billing.billing_country}&billing_city=${steamifyString(CONFIG.billing.billing_city)}&billing_state=${CONFIG.billing.billing_state}&billing_postal_code=${CONFIG.billing.billing_postal_code}&save_my_address=1`
    let totalCost = 0
    let itemsToBuy: any
    itemsToBuy = []
    console.log()
    for(let i = 0; i < results.length; i++) {
        const instock = await checkStock(sessionid, steamloginsecure, data[parseInt(results[i])].listingid, payload)
        if(!instock) {
    ...
    if(itemsToBuy.length != results.length) {
        console.log((chalk.red('Warning! ') + emoji.get('construction') + chalk.bold(results.length - itemsToBuy.length) + ' Items Have Sold Since Calculating the Output!'))
    }
...
async function checkStock(sessionid: string, steamloginsecure: string, listingid: string, data: string): Promise<any> {
    try {
        const url = `https://steamcommunity.com/market/buylisting/${listingid}`;
        const res = await axios.post(url, data, {
          headers: {
        ...
        if(err.response && err.response.data) {
            return err.response.data.message == "There was a problem purchasing your item. The listing may have been removed. Refresh the page and try again."
        }
        await timeout(1000);
        return checkStock(sessionid, steamloginsecure, listingid, data);
      }
...
async function getCookies() {
    ...
    return await new Promise( async (resolve, reject) => {
        ...
        user.on('webSession', (sessionID, cookies) => {
            logo()
            console.log(chalk.green('Got Web Session for ' + chalk.bold(user.accountInfo!.name)))
...
async function getRefresh() {
    return await new Promise( async (resolve, reject) => {
        ...
        if(res.actionRequired) {
            if(res.validActions![res.validActions!.length - 1].type == EAuthSessionGuardType.DeviceCode) {
            ...
            if(res.validActions![res.validActions!.length - 1].type == EAuthSessionGuardType.EmailCode) {
            ...
            if(res.validActions![res.validActions!.length - 1].type == EAuthSessionGuardType.DeviceConfirmation) {
                console.log(chalk.yellow('Confirm your Login in the Steam Guard App! ') + emoji.get('construction'))
            }
            if(res.validActions![res.validActions!.length - 1].type == EAuthSessionGuardType.EmailConfirmation) {
                console.log(chalk.yellow('Check Your Email for a Confirmation Link! ') + emoji.get('construction'))
            }
        }
        session.on('authenticated', async () => {
            logo()
            console.log(chalk.green( chalk.bold(session.accountName) + ' ' + 'Authenticated! ✓'))
